	...
	return(
		<div className={Styles.main}>
			...
			<div className={`${Styles.list} ${np?(Styles.show):(Styles.hide)}`}>
				<div key="demo" className={Styles.list_element}>
						<span className={Styles.element_body}>Title</span>
						<span className={Styles.element_body} style={{color:"#D0D0D0"}}>lorem ipsumvjesi jnfiuawjdian....</span>
				</div>
				{	
					list.map((data,index)=>{
						return(
			...
			<div className={`${Styles.input_form} ${!np?(Styles.show):(Styles.hide)}`}>
				...
				<button onClick={(e)=>{setNp(inp=>!inp);setNote("");setTitle("");}} className={`${Styles.sign} ${Styles.cross_sign} ${!np?(Styles.show):(Styles.hide)}`}>x</button>
			</div>
			<div>
